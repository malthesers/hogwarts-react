...
export default function HouseSelector({ showHouseSelector, setShowHouseSelector }) {
  ...
  function animateCrest() {
    ...
    housesOrder.forEach((house, index) => {
      setTimeout(() => {
        ...
        if (index === 5) setShowHouseSelector(false)            // Close house selector on last iteration
      }, 400 * (index + 1))
    })
  }
  ...
  return (
    <footer className={ (showHouseSelector ? 'h-full' : 'h-20') + ` bg-${theme}-dark` + ' fixed z-20 bottom-0 w-full grid place-content-center duration-500' }>
      ...
      <div onClick={() => setShowHouseSelector(!showHouseSelector)} className={ (showHouseSelector ? 'w-[20rem] sm:w-[30rem]' : '[&>*]:pointer-events-none w-[6rem] mb-20') + ' hogwarts-crest-container grid mx-auto cursor-pointer duration-500'}>
